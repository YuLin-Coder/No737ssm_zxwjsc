$(function() {
	$('#btnUpdate').click(function() {// 保存按钮点击事件
		...
		var data = { "goodsid" : $("#goodsid").val(),"goodsname" : $("#goodsname").val(),"image" : $("#image").val(),"cateid" : $("#cateid").val(),"price" : $("#price").val(),"recommend" : $("#recommend").val(),"special" : $("#special").val(),"storage" : $("#storage").val(),"addtime" : $("#addtime").val(),"hits" : $("#hits").val(),"sellnum" : $("#sellnum").val(),"contents" : $("#contents").val()};
		var loc = $('#basepath').val();// 获取项目根目录
		var url = loc + "goods/xupdateGoods.action";// 获取更新数据Controller地址
