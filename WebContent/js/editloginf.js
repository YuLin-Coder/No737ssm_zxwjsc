$(function() {
	$('#btnUpdate').click(function() {// 保存按钮点击事件
		...
		var data = { "loginfid" : $("#loginfid").val(),"logname" : $("#logname").val(),"ipaddr" : $("#ipaddr").val(),"brower" : $("#brower").val(),"os" : $("#os").val(),"logstatus" : $("#logstatus").val(),"role" : $("#role").val(),"logtime" : $("#logtime").val()};
		var loc = $('#basepath').val();// 获取项目根目录
		var url = loc + "loginf/xupdateLoginf.action";// 获取更新数据Controller地址
