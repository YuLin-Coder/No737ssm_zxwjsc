$(function() {
	$('#btnUpdate').click(function() {// 保存按钮点击事件
		...
		var data = { "complainsid" : $("#complainsid").val(),"usersid" : $("#usersid").val(),"title" : $("#title").val(),"contents" : $("#contents").val(),"addtime" : $("#addtime").val(),"status" : $("#status").val(),"reps" : $("#reps").val()};
		var loc = $('#basepath').val();// 获取项目根目录
		var url = loc + "complains/xupdateComplains.action";// 获取更新数据Controller地址
