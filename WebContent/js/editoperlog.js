$(function() {
	$('#btnUpdate').click(function() {// 保存按钮点击事件
		...
		var data = { "operlogid" : $("#operlogid").val(),"opername" : $("#opername").val(),"methods" : $("#methods").val(),"opers" : $("#opers").val(),"operurl" : $("#operurl").val(),"ipaddr" : $("#ipaddr").val(),"operstatus" : $("#operstatus").val(),"opertime" : $("#opertime").val()};
		var loc = $('#basepath').val();// 获取项目根目录
		var url = loc + "operlog/xupdateOperlog.action";// 获取更新数据Controller地址
