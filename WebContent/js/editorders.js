$(function() {
	$('#btnUpdate').click(function() {// 保存按钮点击事件
		...
		var data = { "ordersid" : $("#ordersid").val(),"ordercode" : $("#ordercode").val(),"usersid" : $("#usersid").val(),"total" : $("#total").val(),"status" : $("#status").val(),"addtime" : $("#addtime").val(),"receiver" : $("#receiver").val(),"address" : $("#address").val(),"contact" : $("#contact").val()};
		var loc = $('#basepath').val();// 获取项目根目录
		var url = loc + "orders/xupdateOrders.action";// 获取更新数据Controller地址
